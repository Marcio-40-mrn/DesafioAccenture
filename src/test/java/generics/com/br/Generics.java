...
public class Generics {
	...
	public void verificaPreencheInput(WebElement elemento, String texto) {

		if (tentaElementoVisivel(elemento, 10)) {
			jsScrollElemento(elemento);
			preecheCampo(elemento, texto);
		}
	}

	public void verificaPreencheComboBox(WebElement elemento, String texto) throws InterruptedException {
		
		if (tentaElementoVisivel(elemento, 10)) {
			jsScrollElemento(elemento);
			selecionaComboBox(elemento, texto);
		}
	}
	
	public void verificaSexo(WebElement elemento, String text) {
		tentaElementoVisivel(elemento, 10);
		jsScrollElemento(elemento);
		if (text == "M") {
			driver.findElement(By.xpath("//section[@class='idealsteps-step']//div[@class='field']//p[@class='group']//input[@value='Male']")).click();
		}else if (text == "F"){
			driver.findElement(By.xpath("//section[@class='idealsteps-step']//div[@class='field']//p[@class='group']//input[@value='Female']")).click();
		}else {
			escreveRelatorio(false, "Erro inesperado item " + text + " nÃo foi localizado", true);
		}
	}
	
	public void verificaSelecionaHobbie(WebElement elemento, String text) {
		tentaElementoVisivel(elemento, 10);
		jsScrollElemento(elemento);
		if(text == Speeding ) {
			driver.findElement(By.xpath("//p[@class='group']//input[@value='Speeding']")).click();
		}else if (text == Bungee Jumping) {
			driver.findElement(By.xpath("//p[@class='group']//input[@value='BungeeJumping']")).click();
		}else if (text == Cliff Diving) {
			driver.findElement(By.xpath("//p[@class='group']//input[@value='CliffDiving']")).click();
		}else if (text == Skydiving) {
			driver.findElement(By.xpath("//p[@class='group']//input[@value='Skydiving']")).click();
		}else if(text == Other) {
			driver.findElement(By.xpath("//p[@class='group']//input[@value='Other']")).click();
		}else {
			escreveRelatorio(false, "Erro inesperado item " + text + " nÃo foi localizado", true);
		}
	}

	public boolean tentaElementoVisivel(WebElement elemento, int timeOut) {
